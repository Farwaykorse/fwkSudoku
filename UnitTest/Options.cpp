...
public:
	...
	TEST_METHOD(T1_memberfunctions)
	{
		...
		//_test(int) const
		static_assert(!noexcept(O_1.test(Value{2})), "test() is NOT noexcept");
		Assert::IsTrue(O_1.test(Value{2}), L"test(value) failed");
		Assert::IsTrue(O_3.test(Value{2}), L"test(value) failed_2");
		Assert::IsFalse(O_3.test(Value{1}), L"test(value) inverse failed");
		try { if (O_1.test(Value{15}) || true) { Assert::Fail(L"test(high) out-of-range expected"); } }
	...
	TEST_METHOD(T3_operators)
	{
		...
		static_assert(noexcept(O_1[Value{2}]), "operator[int] should be no except for reading");
		static_assert(noexcept(O_1[Value{14}]), "operator[out of range] should not throw an exception");
		Assert::IsTrue(O_1[Value{2}], L"reading with operator[int] failed");
		Assert::IsTrue(A_2[Value{2}], L"reading with operator[int] failed_2");
		Assert::IsTrue(A_2[Value{1}] == false, L"reading with operator[int] failed_3");

		///// non-const operators /////
		//NEEDTEST constexpr bool operator[](int)
		static_assert(noexcept(TMP.operator[](Value{0}) = true), "operator[] should be noexcept");
		TMP.clear();
		Assert::IsTrue(TMP[Value{0}] = true, L"operator[] write");
		Assert::IsTrue(TMP[Value{0}] == true, L"operator[] read");
		Assert::IsFalse(TMP[Value{0}].flip(), L"operator[] flip");
		Assert::IsTrue(TMP[Value{0}] == false, L"operator[] read");
